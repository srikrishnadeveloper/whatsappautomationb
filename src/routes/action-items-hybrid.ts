...
// SSE clients for real-time updates (keyed by userId)
...
// SSE endpoint for real-time updates (auth via ?token= query param)
...
router.post('/bulk/complete', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, completed, failed });
  } catch (error: any) {
    res.status(500).json({ success: false, error: error.message });
  }
});
