 ...
 * Uses Supabase when available, falls back to in-memory storage
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: msg, storage: hybridMessageStore.getStorageType() });
  } catch (error: any) {
...
router.post('/', async (req, res) => {
  try {
    const {
      sender, chat_name, content, message_type = 'text',
      classification, decision, priority, ai_reasoning, metadata
    } = req.body;
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: updated, storage: hybridMessageStore.getStorageType() });
  } catch (error: any) {
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Message deleted', storage: hybridMessageStore.getStorageType() });
  } catch (error: any) {
  ...
  try {
    ...
    for (const msg of messages) {
      if (msg.sender && msg.content) {
        const newMsg = await hybridMessageStore.add({
          sender: msg.sender, chat_name: msg.chat_name,
          timestamp: msg.timestamp || new Date().toISOString(),
          content: msg.content, message_type: msg.message_type || 'text',
    ...
    res.status(201).json({ success: true, data: results, count: results.length, storage: hybridMessageStore.getStorageType() });
  } catch (error: any) {
